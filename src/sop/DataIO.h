/*****************************************************************//**
 * \file   DataIO.h
 * \brief  Data input and output functions
 * 
 * \author Qiuchen Qian
 * \date   October 2023
 *********************************************************************/
...
namespace DataIO
{
	...
	void load_sop_data(const fs::path data_dir, const string f_name,
		vector<Circle>& circs);

	void write_stn_zone_vtx(const fs::path data_fdir, const string f_name,
		const size_t t, const vector<StnZone>& stn_zones);

	void write_sop_sol(const fs::path data_fdir, const string f_name,

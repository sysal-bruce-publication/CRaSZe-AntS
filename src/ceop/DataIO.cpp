...
void DataIO::load_ceop_data(const fs::path data_dir, const string f_name,
	vector<Circle>& circs)
{
	// Initialize 
...
void DataIO::write_ceop_sol(const fs::path data_fdir, const string f_name,
...
{
	string token = f_name.substr(0, f_name.find(DOTDELIMITER));
	string of_dir = ((data_fdir / "data" / "ceop" / "sols") / (token + suffix)).string();

...
public:
	...
	StnZoneSrch(const size_t num_iters_szs, const size_t max_circs,
		const double v_uav, vector<Circle>& all_circs) : 
		_num_iters_szs(num_iters_szs), _num_circs(all_circs.size()), 
		_max_circs(max_circs), _v_uav(v_uav)
	{
...
private:
	...
	const size_t _num_circs     = 0;	// Number of all Circles passed in
	const size_t _max_circs     = 0;	// Maximum circle number of a SZ.
	const size_t _num_depots    = 2;	// Number of depots.
	const double _v_uav         = 0;
